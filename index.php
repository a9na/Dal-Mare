...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
    ...
    if ($result->rowCount() > 0) {
        ...
        while ($row = $result->fetch(PDO::FETCH_ASSOC)) { // Use fetch to get each row as an associative array
            ...
            echo '<p class="font-semibold text-lg mt-2">Cijena (€): ' . $row['price'] . '/noć</p>'; // Display the price per night
            echo '<a href="apartmentDetail.php?id=' . $row['id'] . '" class="mt-4 block text-center text-blue-500">Detalji</a>'; // Link to the detailed page of the apartment
            echo '</div>';
            echo '</div>';
        }
    } else {
